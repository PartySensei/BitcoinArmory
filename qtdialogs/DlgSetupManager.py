...
from armoryengine.ArmoryUtils import BTC_HOME_DIR, ARMORY_DB_DIR, ARMORY_HOME_DIR, \
   OS_MACOSX, OS_WINDOWS, USE_TESTNET, USE_REGTEST
...
from qtdialogs.DlgWelcomeScreen import DlgWelcomeScreen
from qtdialogs.qtdialogs import ArmorySplashScreen
...
################################################################################
def createStyledLabel(text, color="white"):
...
def createInputField(width=None, style=None):
   """Create a styled input field with consistent formatting."""
...
def createStyledCombo(width=None, style=None):
   """Create a styled combo box with consistent formatting."""
...
# Consolidated styles for widgets
...
class DlgSetupManager(ArmoryDialog):
   ...
   def initTabs(self):
      ...
      try:
         ...
         self.tabWidget.setContentsMargins(FRAME_PADDING, FRAME_PADDING, FRAME_PADDING, FRAME_PADDING)
   ...
   def createBottomButtonFrame(self):
      ...
      bottomLayout.setContentsMargins(FRAME_MARGIN, FRAME_PADDING, FRAME_MARGIN, FRAME_PADDING)
      bottomLayout.setSpacing(BUTTON_SPACING)
      ...
      # Style the buttons
      self.acceptButton.setStyleSheet(DIALOG_BUTTON_STYLE)
      self.cancelButton.setStyleSheet(DIALOG_BUTTON_STYLE)
   ...
   def accept(self):
      """Handle dialog acceptance and mark setup as completed."""
   ...
   def createCoreTab(self):
      ...
      try:
         ...
         mainLayout = QtWidgets.QVBoxLayout()
         mainLayout.setContentsMargins(FRAME_MARGIN, FRAME_MARGIN, FRAME_MARGIN, FRAME_MARGIN)
         mainLayout.setSpacing(FRAME_PADDING)
   ...
   def createDirectoryFrame(self):
      ...
      dirFrame.setStyleSheet(FRAME_STYLE)
      dirFrameLayout = QtWidgets.QVBoxLayout(dirFrame)
      dirFrameLayout.setContentsMargins(FRAME_PADDING, FRAME_PADDING, FRAME_PADDING, FRAME_PADDING)
      dirFrameLayout.setSpacing(GRID_SPACING)

      # Create grid layout for directory settings
      dirGrid = QtWidgets.QGridLayout()
      dirGrid.setSpacing(GRID_SPACING)
      ...
      dirInputLayout.setSpacing(GRID_SPACING)
      self.satoshiHomePath = createInputField(width=400, style=INPUT_STYLE)
      self.satoshiBrowseButton = createStyledButton(
         self.tr("Browse..."), width=100, style=STANDARD_BUTTON_STYLE)
   ...
   def createCoreSettingsFrame(self):
      ...
      coreFrame.setStyleSheet(FRAME_STYLE)
      coreFrameLayout = QtWidgets.QVBoxLayout(coreFrame)
      coreFrameLayout.setContentsMargins(FRAME_PADDING, FRAME_PADDING, FRAME_PADDING, FRAME_PADDING)
      coreFrameLayout.setSpacing(GRID_SPACING)
      ...
      self.scenarioCombo = createStyledCombo(width=200, style=COMBOBOX_STYLE)
      ...
      self.scenarioCombo.view().setStyleSheet("background-color: #2D2D2D; color: white;")
      self.scenarioCombo.setEditable(False)
      self.scenarioCombo.setFrame(False)

      # Network Mode
      networkLabel = createStyledLabel(self.tr("Network Mode"))
      self.networkModeCombo = createStyledCombo(width=200, style=COMBOBOX_STYLE)
      ...
      self.p2pPortInput = createInputField(width=100, style=INPUT_STYLE)
      self.p2pPortInput.setText("8333")

      # RPC Port (grayed out placeholder)
      rpcPortLabel = createStyledLabel(self.tr("RPC Port"), color="#808080")
      self.rpcPortInput = createInputField(width=100, style=INPUT_STYLE)
      ...
      autoDetectButton = createStyledButton(
         self.tr("Auto-detect Core Settings"), width=200, style=STANDARD_BUTTON_STYLE)
   ...
   def createCoreTab_OLD(self):
      ...
      """)
      dirFrameLayout = QtWidgets.QVBoxLayout(dirFrame)
      dirFrameLayout.setContentsMargins(16, 16, 16, 16)
      ...
      coreDirLabel.setStyleSheet("color: white;")

      dirInputLayout = QtWidgets.QHBoxLayout()
      dirInputLayout.setSpacing(8)
      self.satoshiHomePath = QtWidgets.QLineEdit()
      self.satoshiHomePath.setMinimumWidth(400)
      self.satoshiHomePath.setStyleSheet(INPUT_STYLE)

      self.satoshiBrowseButton = QtWidgets.QPushButton(self.tr("Browse..."))
      self.satoshiBrowseButton.setFixedWidth(100)  # Match Accept/Cancel buttons
      self.satoshiBrowseButton.clicked.connect(self.browseSatoshiHome)
      self.satoshiBrowseButton.setStyleSheet(STANDARD_BUTTON_STYLE)
   ...
   def browseDirDialog(self, lineEdit):
      """Open a directory dialog and set the selected path to the given line edit."""
      dir = QtWidgets.QFileDialog.getExistingDirectory(
         self,
   ...
   def createWalletTab(self):
      ...
      self.armoryDataDirEdit.setStyleSheet(INPUT_STYLE)
      ...
      browseBtn.setStyleSheet(STANDARD_BUTTON_STYLE)
      ...
      walletFrame.setStyleSheet(FRAME_STYLE)
      walletFrameLayout = QtWidgets.QVBoxLayout(walletFrame)
      walletFrameLayout.setContentsMargins(16, 16, 16, 16)
      ...
      self.walletList.setStyleSheet(TREE_WIDGET_STYLE)
      ...
      self.refreshButton.clicked.connect(self.loadWalletList)
      self.refreshButton.setStyleSheet(STANDARD_BUTTON_STYLE)
   ...
   def loadWalletList(self):
      """Load and display the list of available wallets."""
      self.walletList.clear()

      try:
         # Get the wallet directory path
         walletDir = self.armoryDataDirEdit.text()
         if not walletDir or not os.path.exists(walletDir):
            return

         # Scan for wallet files in the directory
         walletFiles = []
         for file in os.listdir(walletDir):
            if file.endswith('.wallet'):
               walletFiles.append(file)

         # Display wallet files in the tree widget
         for walletFile in walletFiles:
            walletPath = os.path.join(walletDir, walletFile)
            walletId = os.path.splitext(walletFile)[0]

            # Create tree widget item
            item = QtWidgets.QTreeWidgetItem()
            item.setText(0, walletId)
            item.setText(1, walletId)  # Use ID as name for now
            item.setText(2, "Standard")  # Default type
            item.setText(3, "Load")  # Default action
         ...
         QtWidgets.QMessageBox.warning(
            ...
            f'Failed to load wallet list: {str(e)}'
         )
   ...
   def createDatabaseTab(self):
      ...
      self.databaseDirEdit.setStyleSheet(INPUT_STYLE)
      ...
      dbDirButton.setStyleSheet(STANDARD_BUTTON_STYLE)
      ...
      scenarioFrame.setStyleSheet(dirFrame.styleSheet())
      ...
      self.databaseScenarioCombo.setStyleSheet(COMBOBOX_STYLE)
      self.databaseScenarioCombo.currentIndexChanged.connect(self.handleDatabaseScenarioChange)
      ...
      self.localDatabaseFrame.setStyleSheet(FRAME_STYLE)
      ...
      self.databaseTypeCombo.setStyleSheet(COMBOBOX_STYLE)
      ...
      self.ramUsageEdit.setStyleSheet(INPUT_STYLE)
      ...
      self.threadCountEdit.setStyleSheet(INPUT_STYLE)
      ...
      self.remoteFrame.setStyleSheet(FRAME_STYLE)
      ...
      self.remoteHostEdit.setStyleSheet(INPUT_STYLE)
      ...
      self.remotePortEdit.setStyleSheet(INPUT_STYLE)
      ...
      self.remoteUserEdit.setStyleSheet(INPUT_STYLE)
      ...
      self.remotePasswordEdit.setStyleSheet(INPUT_STYLE)
      ...
      self.testConnectionButton.clicked.connect(self.testRemoteConnection)
      self.testConnectionButton.setStyleSheet(STANDARD_BUTTON_STYLE)
   ...
   def loadSettings(self, testing=False):
      ...
      if testing:
         self.satoshiHomePath.setText(os.path.normpath(TheSettings.getSettingOrSetDefault('CoreDataDir', '')))
         self.armoryDataDirEdit.setText(os.path.normpath(TheSettings.getSettingOrSetDefault('ArmoryDataDir', '')))
         self.databaseDirEdit.setText(os.path.normpath(TheSettings.getSettingOrSetDefault('DBDir', '')))
      else:
         self.satoshiHomePath.setText(os.path.normpath(TheSettings.getSettingOrSetDefault('CoreDataDir', BTC_HOME_DIR)))
         self.armoryDataDirEdit.setText(os.path.normpath(TheSettings.getSettingOrSetDefault('ArmoryDataDir', ARMORY_HOME_DIR)))
         self.databaseDirEdit.setText(os.path.normpath(TheSettings.getSettingOrSetDefault('DBDir', ARMORY_DB_DIR)))
      ...
      if dbScenario == 'Run Local Database':
         dbType = TheSettings.getSettingOrSetDefault('DBType', 'DB_FULL')
         self.databaseTypeCombo.setCurrentText(dbType)
   ...
   def saveSettings(self):
      ...
      try:
         ...
         if dbScenario == 'Run Local Database':
            TheSettings.set('DBType', self.databaseTypeCombo.currentText())
         ...
         # Update bridge paths in main window
         if self.main:
   ...
   @staticmethod
   def run():
      """Create and show the setup manager dialog with controlled startup sequence.

      Startup Order:
      1. Setup manager dialog appears (modal and focused), the bridge starts
      2. Splash screen appears
      3. Main window is created, after setup manager is closed
      4. Splash screen finishes and Main window is shown

      Returns:
          int: The dialog result (QDialog.Accepted or QDialog.Rejected)
      """
      # First show the welcome screen
      if not DlgWelcomeScreen.showWelcome(None):
         return QtWidgets.QDialog.Rejected

      # 1. Show setup manager (modal and focused) and start bridge
      dlg = DlgSetupManager(testing=False)
      
      # Start the bridge during setup manager display
      try:
         LOGINFO("Starting bridge from setup manager...")
         # Note: Bridge will start in background while setup manager is shown
         # We don't wait for bridge to be fully ready here
         TheBDM.startBridge(getBridgeArgList(), None)  # No callback yet
      except Exception as e:
         LOGEXCEPT("Failed to start bridge: %s", str(e))
      
      # Show setup manager (modal - blocks until user completes setup)
      result = dlg.exec_()
      
      if result != QtWidgets.QDialog.Accepted:
         return QtWidgets.QDialog.Rejected

      # 2. Setup manager closed successfully - now show splash screen
      try:
         LOGINFO("Setup manager completed, showing splash screen...")
         DlgSetupManager._showSplashAndMainWindow()
      except Exception as e:
         LOGEXCEPT("Failed to show splash and main window: %s", str(e))
         return QtWidgets.QDialog.Rejected

      return QtWidgets.QDialog.Accepted

   @staticmethod
   def _showSplashAndMainWindow():
      """Show splash screen, create main window, then finish splash and show main window."""
      ...
      # 3. Create main window (import here to avoid circular imports)
      try:
         from ArmoryQt import ArmoryMainWindow
         LOGINFO("Creating main window...")

         # Create main window with splash reference
         mainWindow = ArmoryMainWindow(splashScreen=splash)

         # Set up bridge callback now that we have main window
         if hasattr(mainWindow, 'networkReadyCallback'):
            # Update bridge with proper callback
            pass  # Bridge already started, just need to connect callback

         # 4. Show main window
         LOGINFO("Showing main window...")
         mainWindow.show()

         # 5. Finish splash screen
         splash.finish(mainWindow)

         LOGINFO("Startup sequence completed successfully")

      except Exception as e:
         LOGEXCEPT("Failed to create or show main window: %s", str(e))
         splash.close()
         raise
